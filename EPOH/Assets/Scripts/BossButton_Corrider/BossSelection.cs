...
public class BossSelection : MonoBehaviour
{
    ...
    public void assignBossInfoToButtons()
    {
        ...
        for (int i = 0; i < boss_buttons.Length; i++)
        {
            ...
            do
            {
                random_boss_index = Random.Range(0, GameManager.bossCnt); // 랜덤한 보스 인덱스 선택
            } while (assigned_boss_indexes.Contains(random_boss_index));

            // GameManager의 boss_clear_info를 확인하여 true인 보스는 배정하지 않음
            while (GameManager.instance.bossClearInfo[random_boss_index])
            {
                random_boss_index = (random_boss_index + 1) % GameManager.bossCnt; // 다음 보스로 이동
            }

            assigned_boss_indexes.Add(random_boss_index); // 배정된 보스 인덱스를 리스트에 추가
            //boss_buttons[i].GetComponentInChildren<TextMeshProUGUI>().text = GameManager.instance.boboss_info[random_boss_index, 0]; // 보스 이름으로 버튼 텍스트 설정
    ...
    public void onBossButtonClick(int button_index, int boss_index)
    {
        ...
        //GameManager.instance. = boss_index; // GameManager에 선택한 보스 정보 전달
