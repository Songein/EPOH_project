...
public class EndingMessages : MonoBehaviour
{
    ...
    private string[] true_ending_messages = 
    {
        "True Ending...!",
        "보스 Hoa를 클리어하고 True Ending을 보는데 성공하셨습니다!",
    };
    ...
    void Start()
    {
        ...
        else if (GameManager.instance.boss_clear_info[5])
        {
            StartCoroutine(displayTrueEndingMessages());
        }
        else
        {
    ...
    IEnumerator displayTrueEndingMessages()
    {
        for (int j = 0; j < true_ending_messages.Length; j++)
        {
            string message = true_ending_messages[j];
            for (int i = 0; i < message.Length; i++)
            {
                b_text.text += message[i]; // Add one character at a time
                yield return new WaitForSeconds(0.1f); // adjust speed here
            }
            if (j < true_ending_messages.Length - 1) // Don't add newline at the end
            {
                b_text.text += "\n"; // Start a new line for each message
            }
