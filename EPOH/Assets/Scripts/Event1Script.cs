...
{
    public GameObject character_panel; // 주인공 Panel을 저장할 변수
    public TMP_Text dialogue_text; // 주인공 TMPro Text 요소를 저장할 변수
    ...
    IEnumerator startCharacterEvent()
    {
        ...
        character_panel.SetActive(true); // 주인공 Panel 활성화
        ...
        dialogue_text.text = "H???: 아무 것도 기억나지 않아…";
        yield return new WaitForSeconds(delay_between_lines);
        ...
        if (tutorial_panel.activeSelf) // 이벤트가 발생하지 않았을 때만 실행
        {
            StartCoroutine(tutorialTextPrint());
        }
